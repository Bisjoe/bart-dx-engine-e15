...
Rectangle2::Rectangle2(bool isActive)
	:Collider(isActive)
	,width(0)
	,height(0)
{
...
//Rectangle2's Constructor:
//We do not accept any negative width or height over here. You shall obey!
//Has there been a consensus on wheter the height would mean the Rectangle2 would go up from X/Y or down from it?
//I'm pretty sure it's assumed that it is the top-left corner based upon the very next constructor...
//You could pretty much do just about anything, heck the x/y could be from the Rectangle2's mid point... ARGH

//Assume everything in the code is based upon this very system. We build Rectangle2s downward, from X/Y, which is the top left point.
/////////////////////////////////////////////////
//(X,Y)//////////////////////////////////////////
////*----------------------*/////////////////////
////|//////////////////////|/////////////////////
...
Rectangle2::Rectangle2(const Vector2D* const topLeft, const Vector2D* const btmRight)
:x(topLeft->x),
y(topLeft->y),
...
height(abs(btmRight->y - this->y))
{
	...
	//The distance between x1 and x2 = width, the distance between y1 and y2 is = height. The Rectangle2'll be valid no matter what.

	//You could argue that you could draw the Rectangle2 first and then simply realign the x/y at the top left corner, or switch the Rectangle2 around...
	...
	//I believe throwing an exception is the general consensus, you'd rather have a valid Rectangle2 than one the user didn't expect.
...
// Please keep in mind if a point was ON one of the Rectangle2's border, it will be considered as WITHIN the Rectangle2.
bool Rectangle2::Contains(const float x, const float y)
{
...
bool Rectangle2::Contains(const Vector2D* const point)
{
	return Contains(point->x, point->y);
}

//For the sake of consistency, the vector is reusing the setposition(points) func and not vice versa.
void Rectangle2::SetPosition(const Vector2D* const vect)
{
	SetPosition(vect->x, vect->y);
	return;
}

//We only have to change the x/y variables to move a Rectangle2, it'll be drawn using its width and height, how nifty is that.
...
void Rectangle2::SetSize(const Vector2D* const vect)
{
	SetSize(vect->x, vect->y);
	return;
}

//Modify the Rectangle2's width and height
void Rectangle2::SetSize(float w, float h)
{
	this->width = w;
	this->height = h;
	return;
...
void Rectangle2::MoveBy(const Vector2D*  const vect)
{
...
void Rectangle2::MoveBy(const float x, const float y)
{
...
// Every axis-aligned Rectangle2-Rectangle2 collisions must respect these  statements to actually be colliding.
// Rect1.TopLeftX < B.BottomRight2
// Rect1.BottomRightX > B.TopLeftX1 
// Rect1.TopLeftY < B.BottomRightY
// Rect1.BottomRightY > B.TopLeftY
bool Rectangle2::CollidesWith(const Rectangle2* const rect) const
{
	bool areColliding;
	areColliding = false;
